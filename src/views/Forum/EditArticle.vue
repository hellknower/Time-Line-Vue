...
import { findArticleWithId,editArticle } from '../../api/forum.js';
...
export default{
    ...
    mounted(){
        ...
        findArticleWithId({articleId:this.articleId}).then((res)=>{
            if(res.success){
                this.articleMessages = res.articles[0];
                this.title = this.articleMessages.articleTitle;
                this.readmeData = this.articleMessages.articleContent;
                for(let item of this.articleKinds){
                    if(item.typeName === this.articleMessages.articleType){
                        this.selectedKind = item.typeValue;
                    }
                }
            }else{
                this.$message({
    ...
    methods:{
        ...
        open(){
            ...
            if(articleTitle.length === 0){
                ...
                editArticle({articleId,articleTitle,articleContent,articleType}).then((res)=>{
                    if(res.success){
                        this.$message({
                            type:'success',
                            message:res.message
